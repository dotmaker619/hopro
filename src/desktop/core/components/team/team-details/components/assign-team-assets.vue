      ...
      <div class="assign-assets__content__body">
        <div v-for="(asset, i) in filteredAllAssets" :key="i" class="assign-assets__content__body__row">
          ...
          <span
            ...
            class="sh-access-tag is-pointer"
          >
...
<script>
  ...
  export default {
    ...
    created() {
      this.selectedAssets = _.keyBy(this.existing, "uid");
      this.get_assets();
    },
    ...
    methods: {
      ...
      async get_assets() {
        this.isLoading = true;
        let { results } = await assetService
          .getAll({
            // query: `exclude_label=${this.$route.params.id}&access=write`,
          })
          .catch((err) => this.handleErrors(err));
        this.allAssets = results;
      ...
      add_assets() {
        teamService
          .patch({
            ...
            body: {
              assets: Object.values(this.selectedAssets),
            },
          })
          .then((res) => {
