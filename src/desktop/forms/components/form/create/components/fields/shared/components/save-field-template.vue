...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      search_results() {
        if (this.field_template) {
          return this.field_template.filter((template) => _.includes(template.name.toLowerCase(), this.search_keyword.toLowerCase()));
        }
      },
    },
    methods: {
      ...mapActions("forms", ["add_field_template"]),
      add_template() {
        this.add_field_template({ body: { name: this.search_keyword, uid: this.id } });
        this.$emit('close');
      },
    },
  };
