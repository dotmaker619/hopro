<template>
  <div v-if="$store.state.app_loading" class="is-flex is-center is-vcenter" style="height: 100vh;">
  ...
  <div id="app" class="is-marginless columns is-mobile" v-else>
    ...
    <div class="root-right column is-paddingless">
      <div class="top m-sticky-navbar">
        <router-view name="top"></router-view>
      </div>
      <div class="root">
        <router-view class="router-root"></router-view>
      </div>
      <div class="sticky-bottom-navbar">
...
import InstallPopup from "@/mobile/shared/components/install-popup.vue";
...
export default {
  ...
  watch: {
    $route() {
      ...
      //     .set(this.$route.path);
      // }
      if(this.$route.query.share) this.$router.push({ name: "share" });
  ...
  beforeDestroy() {
    ...
    this.$store.state.PUSHER.unsubscribe(
      `presence-users-org_${this.$store.state.current_organization.uid}`
    );
  },
  methods: {
    ...mapMutations([
    ...
    async initializeChatStream() {
      let res = await axios.get(
        process.env.VUE_APP_CORE_API_HOST + "/stream/user-token/chat/",
        {
          headers: {
    ...
    async checkOffline() {
      ...
      this.SET_OFFLINE_STATUS(!online);
      setTimeout(this.checkOffline, 10000);
    },
    async initializeServiceworker(status) {
      this.SET_GO_OFFLINE(await get("offline_client"));
    ...
    set_pusher_instance() {
      console.log(this.app_debug_mode);
    ...
    set_online_user_pusher_instance() {
      let channel = this.$store.state.PUSHER.subscribe(
        `presence-users-org_${this.$store.state.current_organization.uid}`
      );
